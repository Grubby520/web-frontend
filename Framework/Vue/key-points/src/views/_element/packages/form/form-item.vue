...
export default {
  ...
  computed: {
    ...
    /* 
    理论上：(inject) this.elForm === this.form
    代码分析：为啥还要通过$parent一层一层向上找到 el-form ?
    */
    form() {
    ...
    fieldValue() {
      ...
      // 深嵌套的场景 prop为'a.b.c', 获取model中prop对应的值；与vue watch差不多，解析监听的属性时；
    ...
    isRequired() {
      ...
      if (rules && rules.length) {
        rules.every((rule) => {
          if (rule.required) {
            isRequired = true; // 有一条规则 required，即为必填
            return false;
          }
          return true;
        });
